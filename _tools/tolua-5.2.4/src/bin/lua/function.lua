...
function Function (d,a,c)
 -- workardound for default arguments like Color(1,1,1) -> Color(1# 1# 1)
 local aa = strsub(a,2,-2)
 local chstart = 0
 local chend = 0
 local subarg = ""
 while true do
   chstart = string.find(aa, "%(", chend+1)
   if chstart == nil then break end
   chend = string.find(aa, "%)", chstart+1)
   if chend == nil then break end
   if chend - chstart > 1 then
     subarg = string.sub(aa, chstart, chend):gsub(",", "#")
     aa = aa:sub(1, chstart-1) .. subarg .. aa:sub(chend+1)
   end
 end

 local t = split(aa,',') -- eliminate braces
 local i = 1
 local l = {n=0}
 while t[i] do
  l.n = l.n+1
  l[l.n] = Declaration(t[i]:gsub("#", ","),'var')
  i = i+1
 end
