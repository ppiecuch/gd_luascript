...
#define MODNAME "memarray"
#define MODTYPE "memarray*"
#define VERSION "0.5"
...
static const void * check_lightuserdata(lua_State *L, int n) {
	if (lua_type(L,n) != LUA_TLIGHTUSERDATA)
		luaL_argerror(L, n, "lightuserdata");
	return (const void *) lua_touserdata(L, n);
}

static memarray_t *memarray_get(lua_State * L, int i) {
	if (luaL_checkudata(L, i, MODTYPE) == NULL)
		luaL_argerror(L, i, MODTYPE);
...
#define PUT_SWAP_2(B, src) \
	luaL_addchar((B), (src)[1]); \
	luaL_addchar((B), (src)[0]);

#define PUT_SWAP_4(B, src) \
	luaL_addchar((B), (src)[3]); \
	luaL_addchar((B), (src)[2]); \
	luaL_addchar((B), (src)[1]); \
	luaL_addchar((B), (src)[0]);

#define PUT_SWAP_8(B, src) \
	luaL_addchar((B), (src)[7]); \
	luaL_addchar((B), (src)[6]); \
	luaL_addchar((B), (src)[5]); \
	luaL_addchar((B), (src)[4]); \
	luaL_addchar((B), (src)[3]); \
	luaL_addchar((B), (src)[2]); \
	luaL_addchar((B), (src)[1]); \
	luaL_addchar((B), (src)[0]);
