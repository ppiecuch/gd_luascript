...
static vector_s vector_rotate_to(vector_s v, vector_s facing) {
	return make_vector(v.x * facing.x + v.y * -facing.y, v.x * facing.y + v.y * facing.x);
}
...
static int separate_by_axis(vector_s axis, lua_Number hw1, const body_s *body1, const body_s *body2, vector_s *out) {
	lua_Number overlap =
...
static int separate_by_axes(const body_s *body1, const body_s *body2, vector_s *out) {
	polygon_s *poly1 = body1->poly;

	for(int i = 0, j = poly1->vertex_count-1; i < poly1->vertex_count; j = i, i++) {
...
static int collision_native__collide(lua_State *L) {
	...
	if(bounding_distance * bounding_distance < distance_squared) {
		...
		if(!separate_by_axes(&body1, &body2, &correction) || !separate_by_axes(&body2, &body1, &correction)) {
			lua_pushboolean(L, 0);
			return 1;
		}
